...
@pytest.fixture
def short_volume_dao():
    short_volume = ShortVolumePercentagesDao(db)
    return short_volume
...
def test_short_volume_dao(short_volume_dao):
    short_volumes = short_volume_dao.find("2016-07-04", "2016-07-04", 500)

    expected_date = datetime(2016,7,4).date()

    assert short_volumes == [
...
def test_short_volume_dao_fail(short_volume_dao):
    short_volumes = short_volume_dao.find("2000-07-11", "3000-01-01", 999999)
    assert short_volumes == []

def test_short_volume_dao_exception(short_volume_dao):
    with pytest.raises(Exception):
        short_volume_dao.find(None)